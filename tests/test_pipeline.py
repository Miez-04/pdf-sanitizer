...
def test_resolve_page_rejects_implausible_model_only_nric():
    ...
    when Tier 1 didn't independently flag it. Since Tier 1's regex is
    comprehensive for these two categories, such a model-only guess
    should be rejected unless it independently passes Tier 1's own
    plausibility check."""
    page = PageTokens(page_num=0, page_width=600, page_height=800)
    page.tokens = [
        _mk_token(0, "2025185153", "O"),  # a student ID, NOT an NRIC
        _mk_token(1, "T5CDCS2304A", "O"),
    ]
    registry = type("R", (), {"__contains__": lambda self, k: False})()
    tier2_labels = ["B-NRIC", "O"]  # model wrongly guesses token 0 is NRIC
    resolve_page(page, registry, tier2_labels)
